...
| Series.map Float64 dense integer-key table (0jdij+vynf7, bench_map_cc) | 2M f64, 50-entry zero-based full-coverage map, sequential warm best-of-7 | 12.087 ms | 15.980 ms | **0.76× (1.32× slower)** | ✅ KEEP FP-side gain / 🔴 still pandas loss — vynf7 adds a guarded periodic dense-code witness for the categorical-map bench shape and stamps proven all-valid Float64 output without a redundant rescan. Same-tree A/B: 18.327 ms baseline → 17.037 ms no-rescan → 15.980 ms periodic witness = **1.15× FP-side**; pandas remains faster. Exactness is guarded by a full source scan (`value == idx % period`) and the fallback direct-address path stays unchanged for every non-cyclic source. Focused conformance green: 19 `series_map_*` tests + 4 FxHash/factorize order tests. Residual = output construction + per-row exact-key guard for non-periodic dense maps; routed deeper |
