# br-frankenpandas-fp-columnar-affine-row-gather-cert-s0yoc affine position certificate proof
...
This is the same measured lever originally recorded as a local
`br-frankenpandas-uza04.45` candidate. Current `origin/main` had already
consumed `.45` through `.57` for other active profile routes, so the
non-conflicting closed bead for this replay is
`br-frankenpandas-fp-columnar-affine-row-gather-cert-s0yoc`. Raw measurement
artifacts retain their original `_uza04_45` filenames, and this proof filename
retains the temporary replay suffix.
...
Do not repeat this local helper rewrite. The remaining profile-backed target is
the repeated per-column affine certificate scan itself:
