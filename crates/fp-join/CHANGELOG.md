...
## [0.1.0](https://github.com/Dicklesworthstone/frankenpandas/releases/tag/fp-join-v0.1.0) - 2026-06-08

### <!-- 0 -->Added

- *(fp-index)* typed missing index label IndexLabel::Null(NullKind) (br-frankenpandas-joeff)
- *(types)* Timedelta::parse accepts ISO-8601 durations like pandas
...
- *(join)* inner identity merge fast-path requires unique keys (cartesian on dups)
- *(join)* deterministic merge_asof group-sort validation (+ FxHashMap)
- *(join)* merge output column order matches pandas (br-691lh)
- *(join)* merge_asof nearest skips exact-equal keys to find true neighbor
...
- *(fp-join)* seek-bound ordered utf8 joins (br-frankenpandas-lr52z)
- *(fp-join)* ordered utf8 inner join positions (br-frankenpandas-53lat)
- *(fp-join)* byte-span build+probe for string-key inner join — 1.30x, zero String clones (br-frankenpandas-i388q)
- *(fp-join)* parallel per-column build in merge_asof output — 2.5x wide asof (br-frankenpandas-fu8f5)
- *(fp-join)* parallel per-column gather in position-based inner merge — 3.2x f64-column joins (br-frankenpandas-j3jnd)
- *(fp-join)* share dense join lazy descriptors
- *(fp-columnar,fp-join)* lazy repeated-slice join lanes
- *(fp-join)* fused dense-i64 right merge builder — 16.81x (br-frankenpandas-4p3ie)
...
- *(fp-join)* row-chunked disjoint-write dense i64 join output fill (br-frankenpandas-6bsw3)
- *(fp-join)* replay dense right buckets from value tape (br-frankenpandas-muis1)
- *(fp-join)* parallel dense i64 join column fill (br-frankenpandas-p2tfu)
- *(fp-join)* lazy unit-range output indexes — 1.70x on 19.5M-row join (br-frankenpandas-r2zi0)
- *(fp-join)* typed dense group-ids for merge_asof(by=) — 2.56x
- *(fp-join)* fill dense outer output columns column-major
- *(fp-join)* close dense right fused output keep
- *(types)* O(n) select_nth for nanmedian/nanquantile — 5.54x
- *(fp-join)* fuse dense all-matched outer output
- *(fp-join)* route many-to-many validate to dense kernels
- *(fp-join)* type-gather dense outer all-present output
- *(fp-join)* type-take all-present left-right joins
- *(fp-join)* type-take all-present outer joins
- *(join)* O(L+R) per-group two-pointer for merge_asof(by=...) — 64.6x
- *(join)* fuse dense i64 inner materialization
- *(join)* dense-packed Int64 multi-key inner join build — 8.7x
- *(join)* typed Int64 gather in general multi-key inner merge — 2.15x
- *(join)* typed Int64 output gather in inner merge — 3.3x
- *(join)* hash-free dense direct-address inner-join build side — 1.8x
- *(join)* skip identity key hash proof
- *(join)* add dense left merge bucket path (br-frankenpandas-89t1y)
- *(join)* add dense outer merge bucket path (br-frankenpandas-59yu9)
- *(join)* add ordered right merge fast path (br-frankenpandas-etyl7)
- *(join)* add ordered outer merge fast path (br-frankenpandas-xxtr4)
- *(join)* add ordered int left merge fast path
- *(join)* add ordered int inner merge fast path
- *(join)* bypass hash for ordered 1:1 inner merges
- *(join)* bypass single-key inner composite keys
- *(join)* inline one-match probe buckets
- *(join)* FxHashMap for merge/join build maps (3.3x, bit-identical)
- *(join)* inline single-key composite keys (SmallVec), zero per-row alloc
- *(join)* fast-path inner merge materialization (br-frankenpandas-938jw)
- *(join)* skip Series right-join probe work (br-frankenpandas-w3lej)
- *(join)* seed merge row vector capacity (br-frankenpandas-hjvd4)
- *(join)* pre-size merge probe maps (br-frankenpandas-23t6z)
- *(join)* skip unsorted merge row-key clones (br-frankenpandas-6f4ea)
- *(join)* skip unused right probe map (br-frankenpandas-an0wx)
- perf-scorecard scaffold honest vs-pandas measurement infrastructure

### <!-- 3 -->Changed

- *(join)* drop unreachable per-group sort validation in merge_asof
- *(fp-frame)* convert where_mask_df_other to delegator over where_cond_df (br-a2sck / fd90.139)

### <!-- 4 -->Documentation

- *(perf)* refresh p2tfu proof ledger
...
- *(fp-join)* add crate-level rustdoc (br-r8sp / fd90.67)
...
- *(join)* differential fuzz guard for int64 merge fast-paths
- *(join)* record identity merge fallback proof (br-frankenpandas-vznz3)
- *(join)* lock merge default _x/_y suffix parity vs pandas 2.2.3
...
- Revert glsam cross-indicator rejection — pandas 2.2.3 actually supports it
...
- fused dense-Int64 builder for the all-matched RIGHT merge route (br-frankenpandas-uza04.34, WIP pass 3/5)
- reject null merge_asof keys
- enforce grouped asof global key order
