...
fn main(){
    let n=3000usize;
    let lk:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,1)%1800) as i64)*104729)).collect();
    let rk:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,2)%1800) as i64)*104729)).collect();
    ...
    let rows:Vec<String>=(0..kk.len()).map(|i| format!("{},{},{}",g(&kk[i]),g(&lvv[i]),g(&rvv[i]))).collect();
