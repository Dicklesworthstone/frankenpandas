...
fn main(){
  ...
  { let (l,r)=build(n);
    for _ in 0..3{black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Outer).unwrap().columns.len());}
    let st=Instant::now(); let mut k=0usize;
    for _ in 0..it{k^=black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Outer).unwrap().columns.len());}
    println!("join_outer_amortized   : {:.3} ms/call (k={k})", st.elapsed().as_secs_f64()*1000.0/it as f64);
  }
  { let mut tot=0.0; let mut k=0usize;
    for _ in 0..3{ let (l,r)=build(n); black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Outer).unwrap().columns.len());}
    for _ in 0..it{ let (l,r)=build(n); let st=Instant::now();
      k^=black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Outer).unwrap().columns.len()); tot+=st.elapsed().as_secs_f64()*1000.0;}
    println!("join_outer_oneshot     : {:.3} ms/call (k={k})", tot/it as f64);
  }
}
