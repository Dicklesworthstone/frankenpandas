...
fn mk(n:usize, seed:u64, payload:&str)->DataFrame{
    ...
    let k0:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,seed)%7) as i64 - 3) * 1_000_003)).collect();
    let k1:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,seed+1)%5) as i64 - 2) * 1_000_033)).collect();
    let k2:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,seed+2)%4) as i64 - 1) * 1_000_037)).collect();
    let v:Vec<Scalar>=(0..n).map(|i| Scalar::Int64((sm(i,seed+3)%1000) as i64)).collect();
...
fn dump(tag:&str, df:&fp_join::MergedDataFrame){
    ...
    let get=|c:&str|->Option<Vec<Scalar>>{ df.columns.get(c).map(|col| col.values().to_vec()) };
    let k0=get("k0").unwrap(); let k1=get("k1").unwrap(); let k2=get("k2").unwrap();
    let lv=get("lv"); let rv=get("rv");
    let f=|v:&Scalar|->String{ match v{Scalar::Int64(x)=>x.to_string(),Scalar::Float64(x)=>if x.is_nan(){"NaN".into()}else{format!("{x:.1}")},Scalar::Null(_)=>"NaN".into(),o=>format!("{o:?}")} };
...
fn main(){
    ...
    for (tag,jt) in [("inner",JoinType::Inner),("left",JoinType::Left),("outer",JoinType::Outer)] {
        dump(tag, &merge_dataframes_on(&left,&right,&["k0","k1","k2"],jt).unwrap());
    }
}
