...
fn mk(n: usize, seed: u64, payload: &str) -> DataFrame {
    ...
    let k0: Vec<Scalar> = (0..n)
        .map(|i| Scalar::Int64(((sm(i, seed) % 700_000) as i64) * 7919))
        .collect();
    let k1: Vec<Scalar> = (0..n)
        .map(|i| Scalar::Int64(((sm(i, seed + 1) % 400) as i64) * 104729))
        .collect();
    let k2: Vec<Scalar> = (0..n)
        .map(|i| Scalar::Int64(((sm(i, seed + 2) % 400) as i64) * 1299709))
        .collect();
    let v: Vec<Scalar> = (0..n)
        .map(|i| Scalar::Float64((sm(i, seed + 3) % 1000) as f64))
        .collect();
...
fn main() {
    ...
    timeit("merge INNER 3 wide-i64 keys (1M)", || {
        std::hint::black_box(
            merge_dataframes_on(&left, &right, &["k0", "k1", "k2"], JoinType::Inner).unwrap(),
        );
    });
}
