...
use fp_index::Index;
use fp_join::{AsofDirection, merge_asof};
use fp_types::Scalar;
fn mk(keys: &[i64], vals: &[i64], vn: &str) -> DataFrame {
    ...
    m.insert(
        ...
        Column::from_values(
            keys.iter()
                .map(|&k| Scalar::Datetime64(base + k * 1_000_000_000))
                .collect(),
        )
        .unwrap(),
    );
...
fn main() {
    ...
    for (tag, d) in [
        ("bwd", AsofDirection::Backward),
        ("fwd", AsofDirection::Forward),
        ("near", AsofDirection::Nearest),
    ] {
        let df = merge_asof(&left, &right, "t", d).unwrap();
        let n = df.index.labels().len();
        let g = |c: &str, i: usize| -> String {
            df.columns
                .get(c)
                .map(|col| match &col.values()[i] {
                    ...
                    Scalar::Datetime64(ns) => {
                        ((ns - 1_600_000_000_000_000_000) / 1_000_000_000).to_string()
                    }
                    o => format!("{o:?}"),
                })
                .unwrap_or("_".into())
