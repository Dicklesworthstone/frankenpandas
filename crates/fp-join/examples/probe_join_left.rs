...
fn main(){
  ...
  { let (l,r)=build(n);
    ...
    println!("join_left_amortized    : {:.3} ms/call (k={k})", st.elapsed().as_secs_f64()*1000.0/it as f64);
  }
  ...
  { let mut tot=0.0; let mut k=0usize;
    ...
    for _ in 0..it{
      let (l,r)=build(n);
      let st=Instant::now();
      k^=black_box(merge_dataframes_on(&l,&r,&["key"],JoinType::Left).unwrap().columns.len());
      tot+=st.elapsed().as_secs_f64()*1000.0;
    }
    println!("join_left_oneshot      : {:.3} ms/call (k={k})", tot/it as f64);
  }
}
