...
fn mkdf(n:usize, seed:u64, vn:&str)->DataFrame{
    ...
    let k1:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,seed)%(n as u64)) as i64)*2654435761)).collect();
    let k2:Vec<Scalar>=(0..n).map(|i| Scalar::Int64((sm(i,seed+10)%1000) as i64)).collect();
...
fn main(){
    ...
    let on=&["k1","k2"];
    timeit("merge2 inner wide-i64x2", || { std::hint::black_box(merge_dataframes_on(&l,&r,on,JoinType::Inner).unwrap()); });
    timeit("merge2 left wide-i64x2", || { std::hint::black_box(merge_dataframes_on(&l,&r,on,JoinType::Left).unwrap()); });
    timeit("merge2 outer wide-i64x2", || { std::hint::black_box(merge_dataframes_on(&l,&r,on,JoinType::Outer).unwrap()); });
}
