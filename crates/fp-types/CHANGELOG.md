...
## [0.1.0](https://github.com/Dicklesworthstone/frankenpandas/releases/tag/fp-types-v0.1.0) - 2026-06-07

### <!-- 0 -->Added

- *(types)* Timedelta::parse accepts ISO-8601 durations like pandas
- *(fp-types)* implement astype Utf8 -> Float64/Int64 parsing
...
- *(types)* nullable Boolean ('boolean') dtype cast is strict (0/1 only)
- *(types)* Timestamp::weekofyear handles ISO 53-week years
- *(types)* Timedelta components use floor-div for negatives like pandas
- *(types)* astype(bool) of NaN yields True like numpy, not null
- *(types)* astype(str) of floats uses Python str(float) incl. scientific notation
- *(types)* astype(bool) uses pandas nonzero truthiness, not 0/1-only
- *(types)* Timedelta::format negative values use Python borrow form
- *(types)* Timedelta::format uses pandas microsecond digits (6, not always 9)
...
- *(fp-types)* harden timedelta time parsing
- *(fp-types)* clear clippy::approx_constant in scalar_from_primitive_types
...
- *(types)* FxHashSet in nannunique (2.0-2.5x, bit-identical)
- *(types)* fuse collect_finite into nanprod/nanptp (2.5-3x, bit-identical)
- *(types)* fuse collect_finite into nansum/nanmean (2.4-2.7x, bit-identical)
...
- *(types)* remove unnecessary unwraps in nancummax/nancummin
...
- add nullable Int64 extension dtype design
- Add Scalar type checking methods
- Add Scalar.to_i64, to_bool, to_str conversion methods
- Add Timestamp.to_julian_date() method
- Add Timestamp.resolution() method
- Add Timedelta component accessor methods
- Add Timestamp.day_of_week and day_of_year aliases
- Add DType helper methods for type checking
- Add Timestamp.toordinal() and fromordinal() methods
- Add Timestamp::now(), utcnow(), today() constructors
- Add Timedelta.floor/ceil/round methods
- Add Timedelta.isoformat() method
- Add Timestamp.to_unit() method
- Add Timestamp.weekofyear/week methods
- Add Timestamp.days_in_month/daysinmonth methods
- Add Timestamp boundary detection methods
- Add Timestamp.dayofweek, dayofyear, quarter methods
- Add Timestamp component accessors
- Add Timestamp.day_name() and month_name() methods
- Add Timestamp.strftime() method
- Add Timestamp.isoformat() method
