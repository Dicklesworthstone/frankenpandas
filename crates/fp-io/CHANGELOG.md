...
## [0.1.0](https://github.com/Dicklesworthstone/frankenpandas/releases/tag/fp-io-v0.1.0) - 2026-06-08
...
- add fp-io html writer surface (frankenpandas-vqjc0.1)
- *(io)* expand DataFrame IO trait wrappers
- *(fp-io)* add pure-string Markdown and LaTeX writers (frankenpandas-vqjc0)
...
- *(fp-io)* gate hdf5 tests behind the hdf5 feature so default `cargo test` compiles
...
- *(fp-io)* CSV parse cache clones DataFrames outside the global lock (concurrency audit, Phase B)
- *(fp-frame,fp-io)* typed null labels for set_index, index_col, pivot and categorical buckets (br-frankenpandas-8m6ay complete)
- *(io)* to_json serializes datetime/timedelta as epoch-millis like pandas
- *(io)* to_latex renders floats with pandas default %.6f (six decimals)
- *(io)* to_csv applies column-uniform datetime format to a DatetimeIndex too
- *(io)* to_csv renders datetime columns like pandas (sub-second + date-only)
- *(io)* read_csv_with_options object-fallback keeps original cell text
- *(io)* read_csv object-fallback columns keep original cell text like pandas
- *(io)* to_csv writes floats as Python str(float) — whole numbers keep .0
- *(io)* read_csv keeps NA/bool/string whitespace; only numeric inference trims
- *(io)* scalar_to_csv writes bool as True/False (pandas parity)
- *(repr)* bool index/column labels render True/False (groupby + io)
- *(fp-io)* reject unsupported SQL schema reads (frankenpandas-w410c)
- *(fp-io)* bound SQL chunk reads
- *(fp-io)* auto-project SQL index_col in columns-list table readers (br-6n0uz)
- *(fp-io)* include SQL index_col in table projections (frankenpandas-fd90.76)
- *(fp-io)* expose SQL inspector backend caps (frankenpandas-fd90.10)
- *(fp-io)* honor SQL table columns in chunked reads (frankenpandas-fd90.72)
...
- *(fp-io)* parallel per-column CSV chunk merge (br-frankenpandas-wktm1)
- *(fp-io)* fused CSV scanner/decimal parser (br-frankenpandas-h96vo)
- *(fp-io)* widen CSV row-block parser (br-frankenpandas-an48z)
- *(fp-io)* range-back CSV default indexes (br-frankenpandas-uza04.37)
- *(fp-io)* cache no-na CSV fast path
- *(io)* cache repeated CSV parses
- *(csv)* fast-path no-na numeric options
- *(csv)* route default options through fast parser
- *(csv)* parse large numeric CSV chunks in parallel
- *(csv)* parse promoted float columns directly
- *(csv)* defer float ASCII scan to fallback
- *(csv)* scan numeric rows as bytes
- *(csv)* build typed numeric columns
- *(fp-io)* speed csv default NA matching
- *(io)* avoid CSV options parser cell allocations (br-frankenpandas-4ejqm)
...
- *(io)* lock JSON non-finite float -> null parity vs pandas 2.2.3
- *(io)* lock CSV scalar inference parity vs pandas 2.2.3
- *(fp-io)* cover all-null SQL dtype hints
- *(fp-io)* cover DataFrame IO extension methods
- *(fp-io)* regression for SQL empty-typed-table dtype preservation [frankenpandas-ex8ec]
- *(fp-io)* SQL identifier quoting regression matrix (frankenpandas-fd90.12)
- *(fp-io)* cover schema SQL projection chunks (frankenpandas-fd90.11)
...
- rust-deps group bump + quick-xml 0.40 migration
...
- Stop Float64 promotion for Int64+null columns (DISC-011 partial)
- test for rejected options.index_col on chunked SQL table reader
- reject options.index_col on SqlChunkIterator entrypoints (br-i8kja)
