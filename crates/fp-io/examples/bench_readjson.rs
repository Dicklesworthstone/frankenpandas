...
fn gen_json(n: usize, salt: u64) -> String {
    let mut s=String::with_capacity(n*60); s.push('[');
    for i in 0..n {
        if i>0 { s.push(','); }
        let a=sm(i,1+salt)%1_000_000; let b=(sm(i,2+salt)%100000) as f64/100.0; let c=sm(i,3+salt)%5000;
        s.push_str(&format!("{{\"id\":{a},\"value\":{b},\"name\":\"item_{c}\"}}"));
    }
    s.push(']'); s
}
fn main(){
    ...
    println!("read_json 300kx3 records COLD: {:.2}ms ({:.1}MB)", best as f64/1e6, bytes as f64/1e6);
}
