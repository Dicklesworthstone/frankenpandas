...
## [0.1.0](https://github.com/Dicklesworthstone/frankenpandas/releases/tag/fp-index-v0.1.0) - 2026-06-08
...
- *(index)* CategoricalIndex sort_values/argsort by category code order, not lexicographic
...
- *(fp-index)* packed-key MultiIndex intersection/difference — ~4.4x
- *(fp-index)* packed-key MultiIndex duplicated/unique/drop_duplicates — ~2.3x
- *(fp-index)* packed-radix MultiIndex argsort/sort_values — ~8.3x
- *(fp-index)* packed-u64 keys for MultiIndex get_indexer / get_indexer_non_unique — ~2.2x
- *(fp-index)* hash-free get_indexer for sorted indexes — ~13.5x align
- *(fp-index)* binary-search DatetimeIndex/TimedeltaIndex get_loc — ~320x
- *(fp-index)* hash-free two-pointer merge for sorted intersection/difference — ~48x
- *(fp-index)* strict-ascending fast path for unique/duplicated/drop_duplicates — unique ~9500x
- *(fp-index)* strict-ascending fast path for has_duplicates — ~50x
- *(fp-index)* share Index label vector via Arc — O(1) clone, 1.67x same-index add
...
- *(index)* seen-set category union for CategoricalIndex set-ops/append — 182x
- *(index)* category->index map for CategoricalIndex codes/argmax/argmin/min/max — up to 528x
- *(index)* O(n+k) hashset membership for CategoricalIndex category ops — up to 80x
- *(frame)* cache unique alignment indexes (br-frankenpandas-typed-columnar-storage-epic-fi6zx.15)
- *(index)* cache Series alignment witness fingerprints
...
- identity-cache index labels for fast equality / dedup
