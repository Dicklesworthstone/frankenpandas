...
## [0.1.0](https://github.com/Dicklesworthstone/frankenpandas/releases/tag/fp-groupby-v0.1.0) - 2026-06-08
...
- *(fp-groupby)* typed null group labels for dropna=False (br-frankenpandas-8m6ay, groupby surface)
- *(groupby)* groupby.sum() of an int column with missing yields Float64
- *(groupby)* groupby.sum() preserves Int64/Bool dtype like pandas
- *(groupby)* groupby.sum() concatenates string columns like pandas
- *(repr)* bool index/column labels render True/False (groupby + io)
- *(fp-groupby)* prevent integer overflow in HyperLogLog estimate
...
- *(fp-groupby)* O(n) select_nth median for dense Int64 groupby — 2.16x
- *(groupby)* dense CSR group-and-sort Median — 7.9x
- *(groupby)* dense 2-D seen-bitset Nunique — 8.2x
- *(groupby)* dense two-pass Var/Std in groupby_agg — 9.3x
- *(groupby)* extend dense streaming groupby_agg to Min/Max/Prod — 3.4x
- *(groupby)* dense streaming groupby_agg for bounded Int64 keys — 3.5x
- *(groupby)* add dense int64 sum buckets
- *(groupby)* FxHashMap for string/sparse-key groupby (2.0x, bit-identical)
...
- *(groupby)* differential fuzz guard for dense int64-key sum path
- *(groupby)* lock groupby var/std float-exact parity vs pandas 2.2.3
