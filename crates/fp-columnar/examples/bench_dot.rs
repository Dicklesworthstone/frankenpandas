//! Bench for `Column::dot` after adding the typed pairwise fast path (sibling of
//! weighted_mean/cov/corr) — folds both raw slices via get_present instead of
//! materializing BOTH columns' Vec<Scalar> + per-Scalar is_missing/to_f64.
//!
//! NEW = col.dot(other). CONTROL = a replica of the pre-typed generic loop over the
//! (cached) values() of both columns ⇒ conservative lower bound (NEW never builds
//! EITHER column's Scalar Vec on a fresh pair). Results asserted equal.
//!
//! Run: cargo run -p fp-columnar --release --example bench_dot -- 5000000 30
...
fn ref_dot(v: &[Scalar], w: &[Scalar]) -> f64 {
    ...
    for (a, b) in v.iter().zip(w.iter()) {
        ...
        let av = match a.to_f64() {
            Ok(x) => x,
            Err(_) => continue,
        };
        let bv = match b.to_f64() {
            Ok(x) => x,
            Err(_) => continue,
        };
...
fn main() {
    ...
    let widata: Vec<i64> = (0..n)
        ...
        .collect();
    let wfdata: Vec<f64> = widata.iter().map(|&x| x as f64 * 0.25).collect();
    ...
    for (label, vcol, wcol) in [("f64_x_i64", &v_f64, &w_i64), ("f64_x_f64", &v_f64, &w_f64)] {
        ...
        for _ in 0..iters {
            let t = std::time::Instant::now();
            let r = ref_dot(vcol.values(), wcol.values());
            best_c = best_c.min(t.elapsed().as_nanos());
            std::hint::black_box(&r);
        }
        assert_eq!(
            vcol.dot(wcol).unwrap().to_bits(),
            ref_dot(&vv, &wv).to_bits(),
            "{label}: NEW != control"
        );
        println!(
            "dot {label:>10} n={n} NEW={:>7.2}ms CONTROL={:>7.2}ms speedup={:.3}x",
            best_t as f64 / 1e6,
            best_c as f64 / 1e6,
