//! Bench for `Column::isin` on a NULLABLE contiguous-Utf8 column (`LazyNullableUtf8`) after
//! adding the nullable arm — probes present byte spans against the Utf8-needle byte set,
//! instead of the generic scan that materializes `Vec<Scalar::Utf8>` (a heap String per row).
//!
//! NEW = col.isin(&needles). CONTROL = faithful COLD generic replica (materialize present
//! strings from the backing + FxHashSet<&str> membership, missing→false, inside the timed loop).
//!
//! Run: cargo run -p fp-columnar --release --example bench_utf8_isin_null -- 5000000 20
...
fn ref_isin_cold(bytes: &[u8], offsets: &[usize], present: &[bool], needle_set: &FxHashSet<&str>) -> Vec<bool> {
    ...
    // (2) membership probe (missing → false).
    owned
        .iter()
        .map(|o| match o {
            Some(s) => needle_set.contains(s.as_str()),
            None => false,
        })
        .collect()
}
...
fn main() {
    ...
    for _ in 0..iters {
        let t = std::time::Instant::now();
        let r = ref_isin_cold(&bytes, &offsets, &present, &needle_set);
        best_c = best_c.min(t.elapsed().as_nanos());
        std::hint::black_box(&r);
    }
    let got = col.isin(&needles).unwrap();
    let want = ref_isin_cold(&bytes, &offsets, &present, &needle_set);
    let gv = got.values();
    assert_eq!(gv.len(), want.len());
    for k in 0..n {
        assert_eq!(gv[k], Scalar::Bool(want[k]), "slot {k} mismatch");
    }
    let n_hit = want.iter().filter(|&&b| b).count();
    println!(
        "isin utf8_nullable n={n} hits={n_hit} NEW={:>7.2}ms CONTROL={:>7.2}ms speedup={:.3}x",
        best_t as f64 / 1e6,
        best_c as f64 / 1e6,
