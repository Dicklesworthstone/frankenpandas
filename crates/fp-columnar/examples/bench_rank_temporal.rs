//! Bench for `Column::rank` on a nullable Datetime64 column — the new temporal
//! typed_radix_perm + typed tie-walk path. Source is a LAZY nullable Datetime64
//! column; before this path rank had NO typed arm for temporal dtypes, so it
//! filtered non-missing and sorted via the O(n log n) na-last Scalar comparator.
//!
...
//!
//! Run: cargo run -p fp-columnar --release --example bench_rank_temporal -- 5000000 12
...
// NaT sentinel for Datetime64 is i64::MIN; a validity-false slot materializes Null.
fn is_missing(v: &Scalar) -> bool {
    matches!(v, Scalar::Null(_)) || matches!(v, Scalar::Datetime64(x) if *x == i64::MIN)
}
...
fn na_last(a: &Scalar, b: &Scalar, ascending: bool) -> Ordering {
    match (is_missing(a), is_missing(b)) {
        ...
        (false, false) => {
            let ord = match (a, b) {
                (Scalar::Datetime64(x), Scalar::Datetime64(y)) => x.cmp(y),
                _ => Ordering::Equal,
            };
...
fn main() {
    ...
    // ns timestamps with ties; every 7th slot missing (~14%). No present-NaT here.
    let data: Vec<i64> = (0..len)
        .map(|i| {
            let h = (i as u64).wrapping_mul(2_654_435_761).wrapping_add(12_345);
            ((h % 50_003) as i64) * 1_000
        })
        .collect();
    ...
    let col = Column::from_datetime64_values_with_validity(data, validity);
    ...
    for method in ["average", "first"] {
        ...
        assert_eq!(ck_t, ck_c, "dt64 {method}: treatment != control");
        println!(
            "rank_temporal dt64 method={method} len={len} iters={iters} \
