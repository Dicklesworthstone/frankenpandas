//! Bench for `Column::filter_by_mask` on a nullable Datetime64/Timedelta64/Bool
//! column with an all-valid Bool mask — the typed nullable temporal/Bool gathers.
//! Source is a LAZY nullable variant; without the fast path it clones a 32-byte
//! Scalar per selected row.
//!
//! A/B via the `FP_NO_NULL_FILTER_TEMPORAL` env gate (set ⇒ Scalar clone path).
//!
//! Run: cargo run -p fp-columnar --example bench_filter_temporal_null --release -- 5000000 30 dt64
...
fn main() {
    ...
    let gated = std::env::var("FP_NO_NULL_FILTER_TEMPORAL").is_ok();
    println!(
        "filter_temporal_null dt={dt} n={n} iters={iters} fast_path={} best={best}ns ({:.3}ms) checksum={checksum}",
        !gated,
        best as f64 / 1e6,
    );
}
