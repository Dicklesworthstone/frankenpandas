//! Bench: nullable-Bool Column::reindex_by_positions (align/reindex/outer-join null-fill).
//! Null-introducing typed arms gated on validity.all(), so nullable Bool fell to the generic
//! Vec<Scalar> gather. NEW = variant-gated raw bool + validity gather.
//! Run: cargo run -p fp-columnar --release --example bench_reindex_boolnull -- 5000000 20
...
fn main() {
    ...
    let col_eager = Column::new(fp_types::DType::Bool, col_sc).unwrap();
    ...
    println!(
        "reindex nullable-Bool n={n} NEW={:.2}ms GENERIC={:.2}ms ({:.1}x)",
        best_new as f64 / 1e6,
        best_ref as f64 / 1e6,
