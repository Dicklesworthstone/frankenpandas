...
## [0.1.0](https://github.com/Dicklesworthstone/frankenpandas/releases/tag/fp-columnar-v0.1.0) - 2026-06-08
...
- satisfy clippy on integrated perf branch
- *(validation)* keep scoped all-target gates green
- *(columnar)* Column::new rejects lossy float->int like pandas constructor
- *(columnar)* resolve take_positions test conflict
- *(columnar)* interpolate forward-fills trailing NaN like pandas
- *(columnar)* from_f64_values marks NaN as missing like the Scalar path
- *(columnar)* astype(int64) truncates finite floats toward zero like pandas
- *(columnar,frame)* Bool.diff() is XOR (pandas 2.2.3), not numeric subtraction
- *(columnar)* Column::around uses banker's rounding to match np.around
- *(columnar)* int**int stays int64, negative integer exponent raises (3w0xn)
- *(columnar)* clamp slice length overflow (br-frankenpandas-qkcdm)
- *(fp-columnar)* promote int FloorDiv/Mod when any right value is zero
- *(columnar)* handle infinity in python_mod_f64 (frankenpandas-review-inf01)
- *(columnar)* match pandas modulo signs (frankenpandas-cod1m01)
...
- *(fp-columnar)* Arc-share LazyAllValid Int64/Float64/Bool buffers — O(1) typed-numeric clone (~30000x)
- *(fp-columnar)* zero-copy LazyUtf8Slice view for ordered-UTF8 inner-join take — 2.09x
- *(fp-columnar)* Arc-share LazyContiguousUtf8 buffers — O(1) string-column clone (~52000x)
- *(fp-columnar)* preserve typed Int64/Bool lazy backing across Column::clone — groupby ~8.7x
- *(fp-join)* bulk fixed-width utf8 equal windows (br-frankenpandas-jbyuc.1.1)
- *(fp-columnar)* vectorized all-valid fast path for f64 binary ops — 1.5x series add
- *(fp-join)* cache utf8 strict witnesses (br-frankenpandas-483i5)
...
- *(fp-columnar)* contiguous-Utf8 gather in Column::take_positions — 3.2x string-column filter (br-frankenpandas-nl1tw)
- *(fp-frame)* fast-sort contiguous Utf8 keys (br-frankenpandas-vecff)
- *(fp-columnar)* string argsort reads contiguous Utf8 buffer — drops 32MB Scalar materialization on sort-after-transform (br-frankenpandas-vecff)
- *(fp-join)* share dense join lazy descriptors
- *(fp-columnar)* skip all-valid validity bitmap allocation
...
- *(fp-columnar,fp-join)* lazy repeated-slice join lanes
- *(fp-join)* fused dense-i64 outer merge builder — 17.77x (br-frankenpandas-343ho)
- *(fp-join)* fused dense-i64 left merge builder — 17.65x (br-frankenpandas-7wxoc)
- *(fp-columnar)* typed nullable gather for reindex_by_positions missing branch (br-frankenpandas-lt5qx)
- *(fp-columnar,fp-join)* typed Float64-promote reindex for null-introducing join gathers (br-frankenpandas-1bvcl)
- *(fp-join)* lazy repeat-run left lanes for high-fanout dense i64 inner merge
- record rejected fused mask filter; filter_bool gap is closed (br-frankenpandas-fi6zx)
- *(fp-columnar)* typed all-valid Bool output for isin/in1d
- *(fp-columnar)* complete SipHash->FxHash migration for dedup/set-op family
- *(fp-columnar)* stable MSD byte-radix argsort for all-valid Utf8 columns
- *(columnar)* O(n) stable-radix Float64 Column::rank — 10.7x
- *(columnar)* dense direct-address Column::mode for bounded Int64 — 20.4x
- *(types)* O(n) select_nth for Timedelta median/quantile arms — 6.7x
- *(columnar)* O(n) counting-sort Column::rank for bounded Int64 — 11x
- *(columnar)* dense direct-address Column::nunique for bounded Int64 — 17.7x
- *(columnar)* dense direct-address Column::unique for bounded Int64 — 39.6x
- *(columnar)* O(N+M) hash-set membership for np set-ops — 702x
- *(columnar)* O(N log B) binary-search bin assignment for Column::histogram — 14x
- *(types)* O(n) select_nth for nanmedian/nanquantile — 5.54x
- *(columnar)* typed Float64/Int64 fast path for prod/nan_to_num/sinc — 4.14x
- *(columnar)* typed two-buffer fast path for 9 binary elementwise ufuncs — 2.53x
- *(columnar)* typed dtype-aware fast paths for rint/neg/square/exp2/signbit/sign — 6.72x
- *(columnar)* typed Float64/Int64 fast path for 23 transcendental ufuncs — 3.48x
- *(columnar)* typed Float64/Int64 fast path for Column::floor/ceil/trunc — 4.91x
- *(columnar)* typed Float64 fast path for Column::round — 5.49x
- *(columnar)* typed prefix-scan cumsum/cumprod/cummax/cummin — ~5.6x
- *(columnar)* dense-membership-bitset isin for bounded Int64 — ~35x
- *(columnar)* typed column-vs-column comparison kernel — ~8x
- *(columnar)* typed-input read in binary arithmetic kernel — ~4.1x
- *(columnar)* typed Float64 var/std reductions — ~14x
- *(columnar)* typed Float64/Int64 min/max reductions — ~14.5x
- *(columnar)* typed Float64 sum/mean reductions — ~14x
- *(columnar)* typed numeric astype Int64<->Float64 — ~32x
- *(columnar)* typed nullable Float64 gather in take_positions — 2.8x
- *(columnar)* typed clip over contiguous buffer — ~22x
- *(columnar)* typed branchless select in where/mask — ~25x
- fuse monotone Series alignment (br-frankenpandas-uza04.22)
- *(columnar)* read typed bool slice in Column::filter_by_mask — ~13x
- *(columnar)* typed Int64 gather in Column::take_positions — 2.3x
- retain nullable Float64 aligned output (br-frankenpandas-uza04.19)
- bypass exact-index Series alignment maps (br-frankenpandas-uza04.18)
- *(columnar)* keep all-valid binary outputs typed (br-frankenpandas-uza04.17)
- *(columnar)* avoid duplicate typed backing
- *(fp-columnar)* fast-path all-present reindex positions
- *(columnar)* typed Column::abs fast path — 4-6x
- *(columnar)* hash-free direct-address factorize codes — 2.67-3.22x
- *(frame)* route user-facing Series.sort_values through typed radix — 2.40x
- *(columnar)* hash-free direct-address dedup for duplicated/drop_duplicates — 4.8-12.3x
- *(columnar)* typed LSD radix sort for sort_values/argsort — 2.74x
- *(columnar)* typed f64/i64 fast path for Column::compare_scalar
- *(columnar)* typed gather for Column::filter_by_mask — 3.25x
- *(frame)* typed Float64 reduction for Series.sum — 5.02x
- *(series)* reuse aligned float buffers
- *(csv)* build typed numeric columns
- *(columnar)* fuse aligned Float64 series arithmetic
- *(columnar)* defer Float64 clone scalars
- *(columnar)* defer Float64 gather scalars
- *(columnar)* clone dense columns from typed cache
- *(columnar)* drop cloned typed cache (br-frankenpandas-typed-columnar-storage-epic-fi6zx.2)
- cache primitive column data for gathers
- specialize primitive column gathers
- *(frame)* fuse aligned float series arithmetic
- *(columnar)* skip validity rebuild for dense gathers (br-frankenpandas-sfysu)
...
- add comprehensive nullable Int64/Bool dtype tests
...
- bench+golden harnesses for value_counts and fp-io

### Fp-columnar

- O(N) hash tally for Column::value_counts
- Add Column math functions neg/sqrt/exp/log/log10/log2
- Add Column.itemsize() method
