...
- *(fp-columnar)* parallelize MSD byte-radix fan-out — str_sort 1.46x / str_groupby 1.30x, bit-identical
- *(fp-frame)* sort-based str-groupby grouping — one MSD byte-radix replaces hash+label-radix+from_utf8, 1.47x, bit-identical
- *(fp-columnar)* lazy strided float64 filter rows
