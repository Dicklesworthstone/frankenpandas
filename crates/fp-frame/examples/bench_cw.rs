...
fn mks(n:usize, off:i64, seed:u64, withnull:bool)->Series{
    ...
    let v:Vec<Scalar>=(0..n).map(|i| if withnull && sm(i,3)%3==0 {Scalar::Null(fp_types::NullKind::Null)} else {Scalar::Float64((sm(i,seed)%1000) as f64)}).collect();
    Series::new("s", idx, Column::from_values(v).unwrap()).unwrap()
}
fn main(){
    let n=1_000_000usize;
    let a=mks(n,0,1,true); let b=mks(n,500_000,2,false);
    timeit("Series.combine_first unaligned (1M)", || { std::hint::black_box(a.combine_first(&b).unwrap()); });
}
