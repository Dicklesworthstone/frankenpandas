...
fn ref_shift(vals: &[Scalar], keys: &[Scalar], periods: i64) -> Column {
    ...
    for (i, k) in keys.iter().enumerate() {
        let kk = format!("{k:?}");
        groups.entry(kk.clone()).or_insert_with(|| { order.push(k.clone()); Vec::new() }).push(i);
    }
    ...
    for (_kk, indices) in &groups {
        ...
        for (idx, slot) in t.iter_mut().enumerate() {
            ...
            if src >= 0 && (src as usize) < m { *slot = gv[src as usize].clone(); }
        }
        for (local, &si) in indices.iter().enumerate() { out[si] = t[local].clone(); }
    }
...
fn main(){
    ...
    for (label, nullable) in [("allvalid", false), ("nullable", true)] {
        let n=5000usize; let card=23usize;
        let keys:Vec<Scalar>=(0..n).map(|i| Scalar::Int64((sm(i,1)%card as u64) as i64)).collect();
        let iv:Vec<Scalar>=(0..n).map(|i| {
            if nullable && sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%100) as i64 - 50)}
        }).collect();
        ...
        for periods in [1i64, 2, 3, -1, -2, 5] {
            ...
            let fc=fast.column();
            let fv=fc.values(); let rv=refc.values();
            let dt_ok = fc.dtype()==refc.dtype();
            let eqv = |a:&Scalar,b:&Scalar| -> bool {
                if a.is_missing() && b.is_missing() { return true; } // missing-equivalent (NullKind canonicalized)
                format!("{a:?}")==format!("{b:?}")
            };
            let mut real_diffs=0; let mut kind_diffs=0;
            for i in 0..n {
                if !eqv(&fv[i],&rv[i]) { real_diffs+=1; }
                else if format!("{:?}",fv[i])!=format!("{:?}",rv[i]) { kind_diffs+=1; }
                // also verify validity bit agrees exactly
                let fmiss=fv[i].is_missing(); let rmiss=rv[i].is_missing();
                if fmiss!=rmiss { real_diffs+=1; }
            }
            if !dt_ok || real_diffs>0 {
                fail+=1;
                println!("FAIL {label} p={periods}: dtype fast={:?} ref={:?} real_diffs={real_diffs} kind_only={kind_diffs}/{n}", fc.dtype(), refc.dtype());
                for i in 0..n { if !eqv(&fv[i],&rv[i]) { println!("  first real diff @{i}: fast={:?} ref={:?}", fv[i], rv[i]); break; } }
            } else {
                println!("OK {label} p={periods} (dtype {:?}, {n} rows; {kind_diffs} missing-slots canonicalized NaN->Null, values+validity identical)", fc.dtype());
            }
        }
    }
