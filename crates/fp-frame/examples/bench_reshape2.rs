...
fn main(){
    ...
    m.insert("id".into(), Column::from_values((0..n).map(|i| Scalar::Int64(i as i64)).collect()).unwrap()); o.push("id".to_string());
    for c in 0..6 { let nm=format!("v{c}"); m.insert(nm.clone(), Column::from_values((0..n).map(|i| Scalar::Float64((sm(i,c as u64+1)%1000) as f64)).collect()).unwrap()); o.push(nm); }
    ...
    timeit("df.melt (500k x 6 valcols)", || { std::hint::black_box(df.melt(&["id"], &vrefs, None, None).unwrap()); });
}
