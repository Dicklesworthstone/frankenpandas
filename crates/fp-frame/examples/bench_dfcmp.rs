...
fn main(){
    ...
    let a=mkdf(n,0); let bs=mkdf(n,500_000);
    timeit("df.gt unaligned (5col,1M)", || { std::hint::black_box(a.gt(&bs).unwrap()); });
    timeit("df.eq unaligned (5col,1M)", || { std::hint::black_box(a.eq(&bs).unwrap()); });
}
