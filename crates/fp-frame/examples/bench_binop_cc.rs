...
fn main() {
    ...
    let a = Series::from_values("a", labels.clone(), (0..n as i64).map(|x| Scalar::Float64(x as f64 * 1.5)).collect()).unwrap();
    let b = Series::from_values("b", labels, (0..n as i64).map(|x| Scalar::Float64(x as f64 + 0.5)).collect()).unwrap();

    let add = best(iters, || { std::hint::black_box(a.add(&b).expect("add")); });
    let mul = best(iters, || { std::hint::black_box(a.mul(&b).expect("mul")); });
    let gt = best(iters, || { std::hint::black_box(a.gt(&b).expect("gt")); });
