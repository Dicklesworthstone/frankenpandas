...
fn main(){
    ...
    for j in 0..3 { let nm=format!("c{j}"); let v:Vec<Scalar>=(0..n).map(|i| Scalar::Float64((sm(i,j)%1000000) as f64)).collect(); m.insert(nm.clone(),Column::from_values(v).unwrap()); order.push(nm); }
    let df=DataFrame::new_with_column_order(idx,m,order).unwrap();
    timeit("df.nlargest(10,c0) 2Mx3", || { std::hint::black_box(df.nlargest(10,"c0").unwrap()); });
    timeit("df.nlargest(1000,c0) 2Mx3", || { std::hint::black_box(df.nlargest(1000,"c0").unwrap()); });
}
