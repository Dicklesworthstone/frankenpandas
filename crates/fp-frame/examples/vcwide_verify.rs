...
fn main(){
    ...
    let n=40000usize;
    let wv:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,1)%(n as u64/8)) as i64)*104729)).collect();
    ...
    let vs:Vec<String>=wv.iter().map(|x| if let Scalar::Int64(v)=x {v.to_string()} else {"NA".into()}).collect();
    writeln!(f,"V\t{}",vs.join(",")).unwrap();
    let r=s.value_counts().unwrap();
    let lbls=r.index().labels(); let col=r.column().values();
    let mut lines=vec![];
    for i in 0..r.len(){
        let l=match &lbls[i]{ fp_index::IndexLabel::Int64(v)=>v.to_string(), o=>format!("{o:?}")};
        let c=match &col[i]{ Scalar::Int64(v)=>v.to_string(), o=>format!("{o:?}")};
