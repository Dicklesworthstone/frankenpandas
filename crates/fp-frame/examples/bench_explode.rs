...
fn main(){
    ...
    let mut bytes=Vec::new(); let mut offs=vec![0usize];
    for i in 0..n { let s=format!("t{}-a,t{}-b,t{}-c", sm(i,0)%1000, sm(i,1)%1000, sm(i,2)%1000); bytes.extend_from_slice(s.as_bytes()); offs.push(bytes.len()); }
    cols.insert("k".to_string(), Column::from_utf8_contiguous(bytes, offs));
    cols.insert("v".to_string(), Column::from_i64_values((0..n as i64).collect()));
    let df=DataFrame::new_with_column_order(idx, cols, vec!["k".into(),"v".into()]).unwrap();
    let mut best=u128::MAX;
    for _ in 0..6 { let t=std::time::Instant::now(); std::hint::black_box(df.explode("k",",").unwrap().len()); best=best.min(t.elapsed().as_nanos()); }
