...
//! Run: cargo run -p fp-frame --example bench_combine_cc --release -- 2000000 30
...
fn main() {
    ...
    let a = Series::from_values("a", labels.clone(),
        (0..n).map(|i| if i % 2 == 0 { Scalar::Float64(i as f64) } else { Scalar::Float64(f64::NAN) }).collect()).unwrap();
    let b = Series::from_values("b", labels,
        (0..n).map(|i| Scalar::Float64(i as f64 * 2.0)).collect()).unwrap();
    ...
    println!("combine_first n={n}: best={best}ns");
}
