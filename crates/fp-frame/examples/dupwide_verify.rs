...
fn main(){
    let n=20000usize;
    let wv:Vec<Scalar>=(0..n).map(|i| Scalar::Int64(((sm(i,1)%(n as u64/3)) as i64)*7919)).collect();
    ...
    let flags:Vec<String>=col.iter().map(|x| match x {Scalar::Bool(b)=> if *b {"1"} else {"0"}, _=>"?"}.to_string()).collect();
