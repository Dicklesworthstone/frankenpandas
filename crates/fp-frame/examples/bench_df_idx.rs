...
fn main() {
    ...
    let mut cols = BTreeMap::new(); let mut order=Vec::new();
    for c in 0..k { let name=format!("c{c}"); cols.insert(name.clone(), Column::from_f64_values((0..n).map(|i| (sm(i,c as u64)%1_000_000) as f64).collect())); order.push(name); }
    let df = DataFrame::new_with_column_order(index, cols, order).unwrap();
    let mut best=u128::MAX;
    for _ in 0..6 { let t=Instant::now(); match op { "idxmax"=>{black_box(df.idxmax().unwrap());}, "idxmin"=>{black_box(df.idxmin().unwrap());}, _=>panic!() } let e=t.elapsed().as_nanos(); if e<best {best=e;} }
