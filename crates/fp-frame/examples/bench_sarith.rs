...
fn mks(n:usize, off:i64)->Series{
    ...
    let v:Vec<Scalar>=(0..n).map(|i| Scalar::Float64((sm(i,1)%1000) as f64)).collect();
    Series::new("s", idx, Column::from_values(v).unwrap()).unwrap()
}
fn main(){
    let n=1_000_000usize;
    let a=mks(n,0); let b_aligned=mks(n,0); let b_shift=mks(n,500_000);
    timeit("s.add aligned (1M)", || { std::hint::black_box(a.add(&b_aligned).unwrap()); });
    timeit("s.add unaligned-shift (1M)", || { std::hint::black_box(a.add(&b_shift).unwrap()); });
}
