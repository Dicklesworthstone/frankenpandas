...
fn main() {
    ...
    cols.insert(
        "a".to_string(),
        Column::from_f64_values((0..n).map(|i| (sm(i, 2) % 2000) as f64).collect()),
    );
    cols.insert(
        "b".to_string(),
        Column::from_f64_values((0..n).map(|i| sm(i, 3) as f64).collect()),
    );
    ...
    for _ in 0..it {
        ...
        match op {
            ...
            "first" => {
                std::hint::black_box(df.groupby(&["k1", "k2"]).unwrap().first().unwrap());
            }
            _ => panic!(),
        };
