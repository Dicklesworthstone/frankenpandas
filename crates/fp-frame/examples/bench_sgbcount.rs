...
fn main(){
    ...
    for (tag,card,stride) in [("bounded-i64",1000usize,1i64),("sparse-i64",200_000usize,1_000_003i64)] {
        ...
        timeit(&format!("sgb.count {tag} nullable-i64"), || { std::hint::black_box(sn.groupby(&k).unwrap().count().unwrap()); });
    }
}
