...
fn main(){
    ...
    for (tag,card) in [("low-1k",1000u64),("med-100k",100_000u64),("high-1M",n as u64/2)] {
        let fv:Vec<Scalar>=(0..n).map(|i| Scalar::Float64(((sm(i,1)%card) as f64)*1.5)).collect();
        let sf=Series::new("sf", idx.clone(), Column::from_values(fv).unwrap()).unwrap();
        timeit(&format!("value_counts f64 {tag}"), || { std::hint::black_box(sf.value_counts().unwrap()); });
    }
}
