...
fn mkdf(n:usize, off:i64, withnull:bool)->DataFrame{
    ...
    for c in 0..5 { let nm=format!("c{c}"); let col:Vec<Scalar>=(0..n).map(|i| if withnull && sm(i,3)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,c as u64+1)%1000) as f64)}).collect(); m.insert(nm.clone(), Column::from_values(col).unwrap()); order.push(nm); }
    DataFrame::new_with_column_order(idx, m, order).unwrap()
}
fn main(){
    let n=1_000_000usize;
    let a=mkdf(n,0,true); let b=mkdf(n,500_000,false);
    timeit("df.combine_first unaligned (5col,1M)", || { std::hint::black_box(a.combine_first(&b).unwrap()); });
}
