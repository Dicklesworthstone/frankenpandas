use fp_frame::Series; use fp_index::Index; use fp_columnar::Column;
...
fn main(){
    ...
    let s=Series::new("s",Index::from_range(0,n as i64,1),Column::from_i64_values_owned((0..n).map(|i|(sm(i,1)%100000) as i64).collect())).unwrap();
    timeit("cut(i64,10) 3M", || { std::hint::black_box(fp_frame::cut(&s,10).unwrap()); });
}
