...
fn main(){
    ...
    let iv:Vec<Scalar>=(0..n).map(|i| if i<3 || sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%100) as i64 - 50)}).collect();
    ...
    let outs:Vec<String>=(0..n).map(|i| match &col[i] { Scalar::Null(_)=>"NA".into(), Scalar::Float64(x)=>format!("{x:.4}"), Scalar::Int64(x)=>format!("{x}.0000"), o=>format!("{o:?}") }).collect();
