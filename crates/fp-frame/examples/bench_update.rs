...
fn main(){
    ...
    let mkdf=|seed:u64,withnull:bool|{ let mut m=BTreeMap::new(); let mut o=vec![]; for c in 0..5 { let nm=format!("c{c}"); let col:Vec<Scalar>=(0..n).map(|i| if withnull && sm(i,seed+c as u64)%3==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,c as u64+seed)%1000) as f64)}).collect(); m.insert(nm.clone(), Column::from_values(col).unwrap()); o.push(nm); } DataFrame::new_with_column_order(idx.clone(), m, o).unwrap() };
    let a=mkdf(1,false); let b=mkdf(2,true);
    timeit("df.update aligned (5col,1M)", || { let mut aa=a.clone(); std::hint::black_box(aa.update(&b).unwrap()); });
}
