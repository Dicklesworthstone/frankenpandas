...
fn main(){
    ...
    let idxk:Vec<Scalar>=(0..n).map(|i| Scalar::Int64((sm(i,1)%200) as i64)).collect();
    let colk:Vec<Scalar>=(0..n).map(|i| Scalar::Int64((sm(i,2)%50) as i64)).collect();
    let val:Vec<Scalar>=(0..n).map(|i| if sm(i,7)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Float64((sm(i,9)%1000) as f64)}).collect();
    ...
    let df=DataFrame::new_with_column_order(idx, map, vec!["r".into(),"c".into(),"v".into()]).unwrap();
    for agg in ["sum","mean"]{
        timeit(&format!("pivot_table {agg} nullable"), || { std::hint::black_box(df.pivot_table("v","r","c",agg).unwrap()); });
    }
}
