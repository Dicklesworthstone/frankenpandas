...
fn dumpvals(col:&[Scalar])->String{
    col.iter().map(|x| match x { Scalar::Null(_)=>"NA".into(), Scalar::Float64(v) if v.is_nan()=>"NA".into(), Scalar::Float64(v)=>format!("{:x}", v.to_bits()), Scalar::Int64(v)=>format!("{v}"), o=>format!("{o:?}") }).collect::<Vec<_>>().join(",")
}
fn build(iv:&[Scalar])->Series{
    let n=iv.len(); let idx=Index::from_range(0,n as i64,1);
    let mut map=BTreeMap::new(); map.insert("iv".into(), Column::from_values(iv.to_vec()).unwrap());
...
fn vstr(iv:&[Scalar])->String{ iv.iter().map(|x| match x { Scalar::Int64(v)=>v.to_string(), _=>"NA".into() }).collect::<Vec<_>>().join(",") }

fn main(){
    let mut f=std::fs::File::create("/tmp/fp_cumprod.txt").unwrap();
    // Case A: random small nullable (overflow to inf, some 0 -> 0)
    let na=5000usize;
    let a:Vec<Scalar>=(0..na).map(|i| if i<2 || sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%12) as i64)}).collect();
    ...
    for (tag,iv) in [("A",&a),("B",&b)]{
        ...
        writeln!(f,"O{tag}\t{}\t{:?}",dumpvals(r.column().values()),r.column().dtype()).unwrap();
    }
    println!("wrote /tmp/fp_cumprod.txt");
    // perf
    let n2=2_000_000usize;
    let pv:Vec<Scalar>=(0..n2).map(|i| if sm(i,7)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%1000) as i64)}).collect();
    let sp=build(&pv);
    timeit("cumprod nullable-i64", || { std::hint::black_box(sp.cumprod().unwrap()); });
}
