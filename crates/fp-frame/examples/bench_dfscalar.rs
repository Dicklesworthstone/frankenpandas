...
fn main() {
    ...
    cols.insert("a".to_string(), Column::from_f64_values((0..n as u64).map(|i| {
    ...
    let df = DataFrame::new_with_column_order(Index::new((0..n as i64).map(IndexLabel::Int64).collect()), cols, vec!["a".into()]).unwrap();
    let mut best=u128::MAX;
    for _ in 0..6 { let t=std::time::Instant::now(); std::hint::black_box(df.add_scalar(1.0).unwrap().shape()); best=best.min(t.elapsed().as_nanos()); }
