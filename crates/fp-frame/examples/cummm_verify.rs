...
fn dumpvals(col:&[Scalar])->String{
    let outs:Vec<String>=col.iter().map(|x| match x { Scalar::Null(_)=>"NA".into(), Scalar::Float64(v)=>format!("{v:.4}"), Scalar::Int64(v)=>format!("{v}.0000"), o=>format!("{o:?}") }).collect();
    outs.join(",")
}
...
fn main(){
    // correctness (small)
    let n=5000usize;
    let iv:Vec<Scalar>=(0..n).map(|i| if i<3 || sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%200) as i64 - 100)}).collect();
    ...
    let mx=s.cummax().unwrap(); let mn=s.cummin().unwrap();
    ...
    writeln!(f,"MAX\t{}\t{:?}",dumpvals(mx.column().values()),mx.column().dtype()).unwrap();
    writeln!(f,"MIN\t{}\t{:?}",dumpvals(mn.column().values()),mn.column().dtype()).unwrap();
    println!("wrote /tmp/fp_cummm.txt max_dtype={:?} min_dtype={:?}", mx.column().dtype(), mn.column().dtype());

    // perf (2M)
    let n2=2_000_000usize;
    let bv:Vec<Scalar>=(0..n2).map(|i| if sm(i,7)%5==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%1000) as i64)}).collect();
    ...
    timeit("cummax nullable-i64", || { std::hint::black_box(sb.cummax().unwrap()); });
    timeit("cummin nullable-i64", || { std::hint::black_box(sb.cummin().unwrap()); });
}
