...
fn ref_diff(vals:&[Scalar], keys:&[Scalar], periods:usize)->Column{
    ...
    for (i,k) in keys.iter().enumerate(){ let kk=format!("{k:?}"); groups.entry(kk.clone()).or_insert_with(||{order.push(kk.clone());Vec::new()}).push(i); }
    ...
    for (_kk,idxs) in &groups {
        ...
        for (li,&si) in idxs.iter().enumerate(){
            if li<periods { continue; }
            let cur=&gv[li]; let prev=&gv[li-periods];
            if cur.is_missing()||prev.is_missing(){ continue; }
            let (Ok(c),Ok(p))=(cur.to_f64(),prev.to_f64()) else { continue; };
...
fn main(){
    ...
    for (label,nullable) in [("allvalid",false),("nullable",true)]{
        let n=5000usize; let card=19usize;
        let keys:Vec<Scalar>=(0..n).map(|i| Scalar::Int64((sm(i,1)%card as u64) as i64)).collect();
        let iv:Vec<Scalar>=(0..n).map(|i| if nullable&&sm(i,7)%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((sm(i,9)%200) as i64 - 100)}).collect();
        ...
        for periods in [1usize,2,3,7]{
            ...
            let mut diffs=0;
            for i in 0..n { if format!("{:?}",fv[i])!=format!("{:?}",rv[i]) { diffs+=1; if diffs==1 { println!("  first diff @{i}: fast={:?} ref={:?}", fv[i], rv[i]); } } }
            if !dt_ok||diffs>0 { fail+=1; println!("FAIL {label} p={periods}: dtype fast={:?} ref={:?} diffs={diffs}/{n}", fc.dtype(), refc.dtype()); }
            else { println!("OK {label} p={periods} (dtype {:?}, {n} rows)", fc.dtype()); }
        }
    }
