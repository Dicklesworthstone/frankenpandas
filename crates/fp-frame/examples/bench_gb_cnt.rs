...
fn main() {
    ...
    let op = a.get(3).map(String::as_str).unwrap_or("cumcount");
    ...
    for _ in 0..it {
        ...
        match op {
            "cumcount" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().cumcount().unwrap());
            }
            "ngroup" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().ngroup().unwrap());
            }
            "size" => {
                std::hint::black_box(df.groupby(&["k"]).unwrap().size().unwrap());
            }
            _ => panic!(),
        };
