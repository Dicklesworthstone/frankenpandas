...
fn main(){
    ...
    let a:Vec<Scalar>=(0..12).map(|i| if i%4==0 {Scalar::Null(NullKind::Null)} else {Scalar::Int64((i*10) as i64)}).collect();
    ...
    for op in ["sum","mean","max","min"] {
        ...
        let r=match op {"sum"=>g.sum(),"mean"=>g.mean(),"max"=>g.max(),_=>g.min()}.unwrap();
