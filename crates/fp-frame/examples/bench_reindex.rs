...
fn main(){
    ...
    for c in 0..5 { let nm=format!("c{c}"); let col:Vec<Scalar>=(0..n).map(|i| Scalar::Float64((sm(i,c as u64+1)%1000) as f64)).collect(); m.insert(nm.clone(), Column::from_values(col).unwrap()); order.push(nm); }
    let df=DataFrame::new_with_column_order(idx, m, order).unwrap();
    let new_labels:Vec<fp_index::IndexLabel>=(500_000i64..1_500_000).map(fp_index::IndexLabel::Int64).collect();
    timeit("df.reindex shifted (5col,1M)", || { std::hint::black_box(df.reindex(new_labels.clone()).unwrap()); });
}
