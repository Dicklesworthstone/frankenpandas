...
- *(fp-frame)* online rolling std var
- *(fp-frame)* typed all-valid f64 fast path for rolling var/std/skew/kurt/sem/prod — 5-7.5x
- *(fp-frame)* O(n) online rolling sum/mean — ~40x (br-frankenpandas-slt1p)
...
- *(fp-frame)* FP-vs-pandas corr differential tool — resolves fgy9g (no divergence)
