...
#[test]
fn live_oracle_series_update_with_overlap() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SUPDATE",
        "case_id": "series_update_with_overlap",
        "mode": "strict",
        "operation": "series_update",
        "oracle_source": "live_legacy_pandas",
        "left": {
            "name": "vals",
            "index": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_update test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let actual = series.update(&other).expect("series update");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_to_frame_unnamed() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STOFRAME",
        "case_id": "series_to_frame_unnamed",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_to_frame test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_series_to_arrow_round_trip_int() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SARROWRT-INT",
        "case_id": "series_to_arrow_round_trip_int",
        "mode": "strict",
    ...
    let actual = super::series_from_arrow_array(
        series.name(),
...
#[test]
fn live_oracle_series_to_arrow_round_trip_string() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SARROWRT-STR",
        "case_id": "series_to_arrow_round_trip_string",
        "mode": "strict",
        "operation": "series_to_arrow_round_trip",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!(
            "live pandas unavailable; skipping series_to_arrow_round_trip string test: {message}"
        );
        return;
    }
    let expected = expected_result.expect("live oracle expected");
...
#[test]
fn live_oracle_dataframe_diff_axis0_periods_1() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDIFF-AXIS0",
        "case_id": "dataframe_diff_axis0_periods_1",
        "mode": "strict",
        "operation": "dataframe_diff",
        "oracle_source": "live_legacy_pandas",
        "diff_periods": 1,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_diff axis=0 test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame
        .diff(fixture.diff_periods.unwrap_or(1))
        .expect("diff axis=0");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_shift_periods_1() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFSHIFT",
        "case_id": "dataframe_shift_periods_1",
        "mode": "strict",
        "operation": "dataframe_shift",
        "oracle_source": "live_legacy_pandas",
        "shift_periods": 1,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_shift test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame
        .shift(fixture.shift_periods.unwrap_or(1))
        .expect("shift");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}
...
#[test]
fn live_oracle_dataframe_rolling_mean_window_3() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFROLLINGMEAN",
        "case_id": "dataframe_rolling_mean_window_3",
        "mode": "strict",
        "operation": "dataframe_rolling_mean",
        "oracle_source": "live_legacy_pandas",
        "window_size": 3,
        "frame": {
            "index": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_rolling_mean test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.rolling(3, None).mean().expect("rolling mean");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_rolling_mean_window_2_min_periods_1() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFROLLINGMEAN-MP1",
        "case_id": "dataframe_rolling_mean_window_2_min_periods_1",
        "mode": "strict",
        "operation": "dataframe_rolling_mean",
        "oracle_source": "live_legacy_pandas",
        "window_size": 2,
    ...
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!(
            "live pandas unavailable; skipping dataframe_rolling_mean min_periods test: {message}"
        );
        return;
    }
    let expected = expected_result.expect("live oracle expected");
...
#[test]
fn live_oracle_dataframe_rank_axis0_average() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFRANK-AXIS0",
        "case_id": "dataframe_rank_axis0_average",
        "mode": "strict",
        "operation": "dataframe_rank",
        "oracle_source": "live_legacy_pandas",
        "rank_method": "average",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_rank axis=0 test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_dataframe_duplicated_keep_last() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDUP-LAST",
        "case_id": "dataframe_duplicated_keep_last",
        "mode": "strict",
        "operation": "dataframe_duplicated",
        "oracle_source": "live_legacy_pandas",
        "keep": "last",
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame
        .duplicated(None, super::DuplicateKeep::Last)
        .expect("duplicated keep=last");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_duplicated_with_subset() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDUP-SUBSET",
        "case_id": "dataframe_duplicated_with_subset",
        "mode": "strict",
        "operation": "dataframe_duplicated",
        "oracle_source": "live_legacy_pandas",
        "subset": ["a"],
    ...
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!(
            "live pandas unavailable; skipping dataframe_duplicated with subset test: {message}"
        );
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let subset_strings: Vec<String> = fixture.subset.as_ref().expect("subset").to_vec();
    let actual = frame
        .duplicated(Some(&subset_strings), super::DuplicateKeep::First)
...
#[test]
fn live_oracle_series_shift_zero_periods() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SSHIFT-ZERO",
        "case_id": "series_shift_zero_periods",
        "mode": "strict",
        "operation": "series_shift",
        "oracle_source": "live_legacy_pandas",
        "shift_periods": 0,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_shift zero test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.shift(0).expect("shift 0");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_shift_periods_larger_than_length() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SSHIFT-LARGE",
        "case_id": "series_shift_periods_larger_than_length",
        "mode": "strict",
        "operation": "series_shift",
        "oracle_source": "live_legacy_pandas",
        "shift_periods": 5,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_shift large test: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.shift(5).expect("shift large");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_pct_change_with_negatives() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SPCTCHANGE-NEG",
        "case_id": "series_pct_change_with_negatives",
        "mode": "strict",
        "operation": "series_pct_change",
        "oracle_source": "live_legacy_pandas",
        "pct_change_periods": 1,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_pct_change negatives: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.pct_change(1).expect("pct_change");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_pct_change_periods_3() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SPCTCHANGE-P3",
        "case_id": "series_pct_change_periods_3",
        "mode": "strict",
        "operation": "series_pct_change",
        "oracle_source": "live_legacy_pandas",
        "pct_change_periods": 3,
        "left": {
            "name": "vals",
            "index": [
                { "kind": "int64", "value": 0 },
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.pct_change(3).expect("pct_change");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_clip_with_series_bounds() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SCLIP-SERIES",
        "case_id": "series_clip_with_series_bounds",
        "mode": "strict",
        "operation": "series_clip",
        "oracle_source": "live_legacy_pandas",
        "clip_lower_series": {
...
#[test]
fn live_oracle_dataframe_clip_upper_only() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFCLIP-UPPER",
        "case_id": "dataframe_clip_upper_only",
        "mode": "strict",
        "operation": "dataframe_clip",
        "oracle_source": "live_legacy_pandas",
        "clip_upper": 50.0,
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame
        .clip(None, fixture.clip_upper)
        .expect("clip upper only");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_clip_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFCLIP-NULL",
        "case_id": "dataframe_clip_with_nulls",
        "mode": "strict",
        "operation": "dataframe_clip",
        "oracle_source": "live_legacy_pandas",
        "clip_lower": 0.0,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_clip with nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
fn live_oracle_dataframe_nsmallest_keep_last_with_ties() {
    ...
    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        ...
        "case_id": "dataframe_nsmallest_keep_last_with_ties",
        "mode": "strict",
        "operation": "dataframe_nsmallest",
        "oracle_source": "live_legacy_pandas",
        "nlargest_n": 3,
...
#[test]
fn live_oracle_series_constructor_int_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SCONSTRUCTOR-INT",
        "case_id": "series_constructor_int_basic",
        "mode": "strict",
        "operation": "series_constructor",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_constructor int: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_series_constructor_float_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SCONSTRUCTOR-FLOAT",
        "case_id": "series_constructor_float_basic",
        "mode": "strict",
        "operation": "series_constructor",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_constructor float: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_nan_min_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-NANMIN",
        "case_id": "nan_min_basic",
        "mode": "strict",
        "operation": "nan_min",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping nan_min: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let left = fixture.left.as_ref().expect("left");
    let actual = super::nanmin(&left.values);
    super::compare_scalar(&actual, &expected, "nan_min").expect("pandas parity");
}

#[test]
fn live_oracle_nan_max_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-NANMAX",
        "case_id": "nan_max_basic",
        "mode": "strict",
        "operation": "nan_max",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping nan_max: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let left = fixture.left.as_ref().expect("left");
    let actual = super::nanmax(&left.values);
    super::compare_scalar(&actual, &expected, "nan_max").expect("pandas parity");
}

#[test]
fn live_oracle_nan_count_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-NANCOUNT",
        "case_id": "nan_count_basic",
        "mode": "strict",
        "operation": "nan_count",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping nan_count: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let left = fixture.left.as_ref().expect("left");
    let actual = super::nancount(&left.values);
    super::compare_scalar(&actual, &expected, "nan_count").expect("pandas parity");
}

#[test]
fn live_oracle_nan_std_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-NANSTD",
        "case_id": "nan_std_basic",
        "mode": "strict",
        "operation": "nan_std",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping nan_std: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let left = fixture.left.as_ref().expect("left");
    let actual = super::nanstd(&left.values, 1);
    super::compare_scalar(&actual, &expected, "nan_std").expect("pandas parity");
}

#[test]
fn live_oracle_nan_var_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-NANVAR",
        "case_id": "nan_var_basic",
        "mode": "strict",
        "operation": "nan_var",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping nan_var: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_dataframe_melt_with_value_vars_subset() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFMELT-SUBSET",
        "case_id": "dataframe_melt_with_value_vars_subset",
        "mode": "strict",
        "operation": "dataframe_melt",
        "oracle_source": "live_legacy_pandas",
        "melt_id_vars": ["id"],
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_melt subset: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let id_vars: Vec<&str> = fixture
        ...
        .map(String::as_str)
        .collect();
    let value_vars: Vec<&str> = fixture
        .melt_value_vars
        .as_ref()
        .expect("melt_value_vars")
        .iter()
        .map(String::as_str)
        .collect();
    let actual = frame
        .melt(&id_vars, &value_vars, None, None)
        .expect("melt subset");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_where_df_bool_frame() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFWHEREDF",
        "case_id": "dataframe_where_df_bool_frame",
        "mode": "strict",
        "operation": "dataframe_where_df",
        "oracle_source": "live_legacy_pandas",
        "frame": {
            "index": [
                { "kind": "int64", "value": 0 },
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_where_df: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let cond =
        super::build_dataframe(fixture.frame_right.as_ref().expect("frame_right")).expect("cond");
    let other =
        super::build_dataframe(fixture.frame_other.as_ref().expect("frame_other")).expect("other");
    let actual = frame.where_cond_df(&cond, &other).expect("where_df");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_mask_df_bool_frame() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFMASKDF",
        "case_id": "dataframe_mask_df_bool_frame",
        "mode": "strict",
        "operation": "dataframe_mask_df",
        "oracle_source": "live_legacy_pandas",
        "frame": {
            "index": [
                { "kind": "int64", "value": 0 },
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_mask_df: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_dataframe_count_mixed_types() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFCOUNT-MIXED",
        "case_id": "dataframe_count_mixed_types",
        "mode": "strict",
        "operation": "dataframe_count",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_count mixed_types: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.count().expect("count");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_corr_pearson_min_periods_3() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFCORR-MINPER",
        "case_id": "dataframe_corr_pearson_min_periods_3",
        "mode": "strict",
        "operation": "dataframe_corr",
        "oracle_source": "live_legacy_pandas",
        "corr_min_periods": 3,
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame
        .corr_min_periods_with_numeric_only(3, false)
        .expect("corr min_periods");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_describe_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDESCRIBE-NULL",
        "case_id": "dataframe_describe_with_nulls",
        "mode": "strict",
        "operation": "dataframe_describe",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_describe nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_dataframe_value_counts_two_columns() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFVC-2COL",
        "case_id": "dataframe_value_counts_two_columns",
        "mode": "strict",
        "operation": "dataframe_value_counts",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!(
            "live pandas unavailable; skipping dataframe_value_counts two_columns: {message}"
        );
        return;
    }
    let expected = expected_result.expect("live oracle expected");
...
#[test]
fn live_oracle_dataframe_skew_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFSKEW-NULL",
        "case_id": "dataframe_skew_with_nulls",
        "mode": "strict",
        "operation": "dataframe_skew",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_skew nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.skew_agg().expect("skew");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_kurtosis_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFKURT-NULL",
        "case_id": "dataframe_kurtosis_with_nulls",
        "mode": "strict",
        "operation": "dataframe_kurtosis",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_kurtosis nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.kurtosis_agg().expect("kurtosis");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_sem_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFSEM-NULL",
        "case_id": "dataframe_sem_with_nulls",
        "mode": "strict",
        "operation": "dataframe_sem",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_sem nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.sem_agg().expect("sem_agg");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_diff_axis0_negative_periods() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDIFF-NEG",
        "case_id": "dataframe_diff_axis0_negative_periods",
        "mode": "strict",
        "operation": "dataframe_diff",
        "oracle_source": "live_legacy_pandas",
        "diff_periods": -1,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_diff negative: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_dataframe_sort_values_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFSORTVAL-NULL",
        "case_id": "dataframe_sort_values_with_nulls",
        "mode": "strict",
        "operation": "dataframe_sort_values",
        "oracle_source": "live_legacy_pandas",
        "sort_column": "a",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_sort_values nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.sort_values("a", true).expect("sort_values");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_drop_duplicates_keep_last() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDROPDUP-LAST",
        "case_id": "dataframe_drop_duplicates_keep_last",
        "mode": "strict",
        "operation": "dataframe_drop_duplicates",
        "oracle_source": "live_legacy_pandas",
        "keep": "last",
...
#[test]
fn live_oracle_dataframe_set_index_with_int_column() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFSETIDX-INT",
        "case_id": "dataframe_set_index_with_int_column",
        "mode": "strict",
        "operation": "dataframe_set_index",
        "oracle_source": "live_legacy_pandas",
        "set_index_column": "id",
    ...
    let actual = frame
        ...
        .expect("set_index int");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_insert_middle() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFINSERT",
        "case_id": "dataframe_insert_middle",
        "mode": "strict",
        "operation": "dataframe_insert",
        "oracle_source": "live_legacy_pandas",
        "insert_loc": 1,
        "insert_column": "new",
        "insert_values": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_insert: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_dataframe_mode_int_columns() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFMODE-INT",
        "case_id": "dataframe_mode_int_columns",
        "mode": "strict",
        "operation": "dataframe_mode",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_mode int: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
fn live_oracle_dataframe_compare_identical() {
    ...
    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        ...
        "case_id": "dataframe_compare_identical",
        "mode": "strict",
        "operation": "dataframe_compare",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_compare identical: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let right =
        super::build_dataframe(fixture.frame_right.as_ref().expect("frame_right")).expect("right");
    let actual = left.compare(&right).expect("compare");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_rename_columns_three() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFRENAME-3",
        "case_id": "dataframe_rename_columns_three",
        "mode": "strict",
        "operation": "dataframe_rename_columns",
        "oracle_source": "live_legacy_pandas",
        "rename_columns": [
    ...
    let actual = super::execute_dataframe_fixture_operation(&fixture).expect("rename three");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_dataframe_drop_columns_first_only() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFDROPCOLS-FIRST",
        "case_id": "dataframe_drop_columns_first_only",
        "mode": "strict",
        "operation": "dataframe_drop_columns",
        "oracle_source": "live_legacy_pandas",
        "drop_columns": ["a"],
    ...
    let actual = super::execute_dataframe_fixture_operation(&fixture).expect("drop_columns");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_at_time_evening() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SATTIME-EVE",
        "case_id": "series_at_time_evening",
        "mode": "strict",
        "operation": "series_at_time",
        "oracle_source": "live_legacy_pandas",
        "time_value": "20:00:00",
        "left": {
            ...
            "index": [
                { "kind": "utf8", "value": "2024-01-01 09:00:00" },
                { "kind": "utf8", "value": "2024-01-01 20:00:00" },
                { "kind": "utf8", "value": "2024-01-02 14:30:00" },
                { "kind": "utf8", "value": "2024-01-02 20:00:00" }
            ],
            "values": [
                { "kind": "int64", "value": 1 },
                { "kind": "int64", "value": 2 },
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_at_time evening: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_series_factorize_strings() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SFACTORIZE-STR",
        "case_id": "series_factorize_strings",
        "mode": "strict",
        "operation": "series_factorize",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_factorize strings: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let (codes, _) = series.factorize().expect("factorize strings");
    super::compare_series_expected(&codes, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_fillna_string_with_default() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SFILLNA-STR",
        "case_id": "series_fillna_string_with_default",
        "mode": "strict",
        "operation": "series_fillna",
        "oracle_source": "live_legacy_pandas",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_fillna string: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
...
#[test]
fn live_oracle_series_replace_string_to_string() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-SREPLACE-STR",
        "case_id": "series_replace_string_to_string",
        "mode": "strict",
        "operation": "series_replace",
        "oracle_source": "live_legacy_pandas",
        "replace_to_find": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_replace string-to-string: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let pairs: Vec<(super::Scalar, super::Scalar)> = fixture
        .replace_to_find
        .as_ref()
        .expect("replace_to_find")
        .iter()
...
#[test]
fn live_oracle_dataframe_pct_change_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFPCTCHANGE-NULL",
        "case_id": "dataframe_pct_change_with_nulls",
        "mode": "strict",
        "operation": "dataframe_pct_change",
        "oracle_source": "live_legacy_pandas",
        "diff_periods": 1,
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping dataframe_pct_change nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let frame = super::build_dataframe(fixture.frame.as_ref().expect("frame")).expect("frame");
    let actual = frame.pct_change(1).expect("pct_change with nulls");
    super::compare_dataframe_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_str_lower_with_unicode() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRLOWER-UNI",
        "case_id": "series_str_lower_with_unicode",
        "mode": "strict",
        "operation": "series_str_lower",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_str_lower unicode: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.str().lower().expect("str lower unicode");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_str_upper_with_unicode() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRUPPER-UNI",
        "case_id": "series_str_upper_with_unicode",
        "mode": "strict",
        "operation": "series_str_upper",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_str_upper unicode: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.str().upper().expect("str upper unicode");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_str_len_with_unicode_and_empty() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRLEN-UNI",
        "case_id": "series_str_len_with_unicode_and_empty",
        "mode": "strict",
        "operation": "series_str_len",
        "oracle_source": "live_legacy_pandas",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping series_str_len unicode: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(
    ...
    let series = super::build_series(fixture.left.as_ref().expect("left")).expect("series");
    let actual = series.str().len().expect("str len");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_str_isalnum_mix() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRISALNUM",
        "case_id": "series_str_isalnum_mix",
        "mode": "strict",
        "operation": "series_str_isalnum",
        "oracle_source": "live_legacy_pandas",
        "left": {
            "name": "txt",
            "index": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping str isalnum: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
...
#[test]
fn live_oracle_series_str_isnumeric_mix() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRISNUM",
        "case_id": "series_str_isnumeric_mix",
        "mode": "strict",
        "operation": "series_str_isnumeric",
        "oracle_source": "live_legacy_pandas",
        "left": {
            "name": "txt",
            "index": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping str isnumeric: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
...
#[test]
fn live_oracle_series_str_islower_mix() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRISLOWER",
        "case_id": "series_str_islower_mix",
        "mode": "strict",
        "operation": "series_str_islower",
        "oracle_source": "live_legacy_pandas",
        "left": {
            "name": "txt",
            "index": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping str islower: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
...
#[test]
fn live_oracle_series_str_isupper_mix() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRISUPPER",
        "case_id": "series_str_isupper_mix",
        "mode": "strict",
        "operation": "series_str_isupper",
        "oracle_source": "live_legacy_pandas",
        "left": {
            "name": "txt",
            "index": [
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping str isupper: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
...
#[test]
fn live_oracle_series_str_match_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRMATCH",
        "case_id": "series_str_match_basic",
        "mode": "strict",
        "operation": "series_str_match",
        "oracle_source": "live_legacy_pandas",
        "regex_pattern": "h.*",
        "left": {
            "name": "text",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping str match: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
    ...
    let actual = series
        .str()
        .match_regex(fixture.regex_pattern.as_deref().expect("regex_pattern"))
        .expect("match");
    super::compare_series_expected(&actual, &expected).expect("pandas parity");
}

#[test]
fn live_oracle_series_str_rjust_width8() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-STRRJUST",
        "case_id": "series_str_rjust_width8",
        "mode": "strict",
        "operation": "series_str_rjust",
        "oracle_source": "live_legacy_pandas",
        "str_width": 8,
        "str_fillchar": "_",
        "left": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping str rjust: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
...
fn live_oracle_dataframe_apply_product_axis1() {
    ...
    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        ...
        "case_id": "dataframe_apply_product_axis1",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping apply_product: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
...
#[test]
fn live_oracle_dataframe_apply_sem_axis0() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DFAPPLYSEM",
        "case_id": "dataframe_apply_sem_axis0",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping apply_sem: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Series(_)));
...
#[test]
fn live_oracle_dataframe_cov_three_columns() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-COV-3COL",
        "case_id": "dataframe_cov_three_columns",
        "mode": "strict",
        "operation": "dataframe_cov",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df cov 3col: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
...
#[test]
fn live_oracle_dataframe_cov_with_nulls() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-COV-NULLS",
        "case_id": "dataframe_cov_with_nulls",
        "mode": "strict",
        "operation": "dataframe_cov",
        "oracle_source": "live_legacy_pandas",
        "frame": {
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df cov nulls: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_merge_inner_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-MERGE-INNER",
        "case_id": "dataframe_merge_inner_basic",
        "mode": "strict",
        ...
        "frame_right": {
            ...
            "columns": {
                ...
                "right_v": [
                    { "kind": "int64", "value": 220 },
                    { "kind": "int64", "value": 330 }
                ]
            }
        }
    }))
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping merge inner: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_merge_left_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-MERGE-LEFT",
        "case_id": "dataframe_merge_left_basic",
        "mode": "strict",
        "operation": "dataframe_merge",
        "oracle_source": "live_legacy_pandas",
        "join_type": "left",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping merge left: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_merge_outer_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-MERGE-OUTER",
        "case_id": "dataframe_merge_outer_basic",
        "mode": "strict",
        "operation": "dataframe_merge",
        "oracle_source": "live_legacy_pandas",
        "join_type": "outer",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping merge outer: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_merge_right_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-MERGE-RIGHT",
        "case_id": "dataframe_merge_right_basic",
        "mode": "strict",
        "operation": "dataframe_merge",
        "oracle_source": "live_legacy_pandas",
        "join_type": "right",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping merge right: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_groupby_sum_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-GBSUM",
        "case_id": "dataframe_groupby_sum_basic",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df groupby sum: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_groupby_any_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-GBANY",
        "case_id": "dataframe_groupby_any_basic",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df groupby any: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_groupby_all_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-GBALL",
        "case_id": "dataframe_groupby_all_basic",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df groupby all: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_groupby_idxmin_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-GBIDXMIN",
        "case_id": "dataframe_groupby_idxmin_basic",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df groupby idxmin: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
...
#[test]
fn live_oracle_dataframe_groupby_idxmax_basic() {
    let mut cfg = super::HarnessConfig::default_paths();
    cfg.allow_system_pandas_fallback = false;

    let fixture: super::PacketFixture = serde_json::from_value(serde_json::json!({
        "packet_id": "FP-P2D-LIVE-DF-GBIDXMAX",
        "case_id": "dataframe_groupby_idxmax_basic",
        "mode": "strict",
    ...
    let expected_result = super::capture_live_oracle_expected(&cfg, &fixture);
    if let Err(super::HarnessError::OracleUnavailable(message)) = &expected_result {
        eprintln!("live pandas unavailable; skipping df groupby idxmax: {message}");
        return;
    }
    let expected = expected_result.expect("live oracle expected");
    assert!(matches!(&expected, super::ResolvedExpected::Frame(_)));
