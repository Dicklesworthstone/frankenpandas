...
- **Related, and NOT implemented:** pandas **raises** `TypeError: 'DatetimeArray' with dtype datetime64[ns] does not support reduction '<name>'` for datetime64 `sum`, `prod`, `var`, `sem`, and `skew`. FrankenPandas returns a missing value for these rather than inventing one; it deliberately does not add support pandas itself refuses. `std` **is** supported by pandas for datetimes (returning a Timedelta) and is not yet implemented in FP — tracked separately, since it involves a sqrt and needs its own accuracy contract.
